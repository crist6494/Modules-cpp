...
/*   Updated: 2023/10/16 11:47:07 by cmorales         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::signForm(AForm& form)
{
	...
	{
		std::cerr << RED << this->getName() << " couldn’t sign " << form.getName() << " because the bureaucrat's grade is lower than the form's grade to sign" << std::endl << RESET;
	}
}
...
void Bureaucrat::executeForm(const AForm& form)
{
	...
	{
		form.execute(*this);		
		std::cout << BLUE << this->getName() << " executed " << form.getName() << std::endl << RESET;
	}
	catch(AForm::GradeSignedException& e)
	{
		std::cerr << RED << "Error: " << form.getName() << e.what() << std::endl << RESET;
	}
	catch(std::exception& e)
	{
		std::cerr << RED << this->getName() << " couldn’t sign " << form.getName() << " because the bureaucrat's grade is lower than the form's grade to execute" << std::endl << RESET;
	}
}
